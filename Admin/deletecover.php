...
{
	$cn=makeconnection();
	$s="delete from cover  where coverid='" . $_POST["s1"] . "'";
	mysqli_query($cn,$s);
	mysqli_close($cn);
	header ("location: deletecover.php?note=Record deleted");
    }
?>
...
<tr><td colspan="2" class="toptd">Delete Cover</td></tr>
<tr><td class="lefttxt">Select Company Name</td><td><select name="t2" required/><option value="">Select</option>
...
$cn=makeconnection();
$s="select * from cover where patid='" . $_POST["t2"] ."'";
$result=mysqli_query($cn,$s);
$r=mysqli_num_rows($result);
//echo $r;
